...
13c. **相邻区块间距**：页面上任何两个相邻的独立区块（标题区、tab 栏、内容区、卡片、列表、按钮组等）之间必须有明确间距，不允许贴在一起。使用 `margin-bottom` 或 `gap`（`16-24px` 常规区块，`24-32px` 较大区块）。检查方式：视觉上每个区块边缘应有清晰的留白分隔，tab 栏与下方内容、按钮与相邻元素、标题与正文之间都不应紧贴。**表单步骤/分组容器内部的独立卡片或字段块之间也要保持间距**（如 `.book-card + .book-card { margin-top: 18px; }`、`.book-card { margin-bottom: 18px; }` 或步骤容器 `gap`），不能因为外层容器已有 `gap` 就忽略内部相邻元素间距；**步骤/表单底部的操作按钮（下一步、提交）与上方最后一个内容块之间必须有间距**（卡片 `margin-bottom` 或按钮 `margin-top`）。
13d. **汇总/确认卡不得压缩在窄侧栏**：订单预览、结算汇总、确认信息等含多行信息的卡片（房型、日期、晚数、数量、金额、总价）必须有足够宽度完整展示，不允许压进 `< 340px` 的侧栏导致信息局促、换行错乱或显示不全；信息较多时优先整卡全宽或分栏展示。
13e. **流程结果/成功态必须是完整全宽面板**：表单流程（预订、下单、提交）的成功/结果页要有明确的容器（背景、圆角、阴影）并居中内容，不允许以光秃秃的窄居中文本漂浮在页面上、看起来只占一半。成功态出现时隐藏不再相关的表单、步骤条、汇总卡等元素，只保留结果信息与下一步动作。
...
15. **状态圆点/徽标样式统一**：状态指示点（`dot`、状态圆点）必须：
   ...
   - **每个带圆点的徽标变体必须同时声明圆点背景色**：`.badge.xxx .dot { background: <变量>; }`。只有 `.badge .dot { width/height }` 而缺少对应变体的圆点背景色，圆点会「有宽高但不可见」，判定为缺陷
   - **禁止引用未定义的 CSS 变量**：页面中每个 `var(--xxx)` 都必须在 `:root` 中有定义。徽标/状态色优先使用语义变量（`--success` / `--warning` / `--danger` / `--info`）；确需业务语义变量（如 `--hot`）时，背景色（`--hot-bg`）、前景色与圆点色必须在同一 `:root` 内完整定义。交付前通读所有 `var()` 引用逐一核对定义
...
16b. **表格操作列按钮对齐**：操作列中每行的按钮结构必须一致，不能因某行缺少某操作导致图标错位：
   - 条件渲染的按钮（如"登记回款"只在未结清行出现）缺失时，用等宽占位（`<span class="action-placeholder">`）替代，让该行的其余图标（如眼睛图标）与其他行保持在同一垂直线上
   - 占位宽度与缺失按钮宽度一致（如按钮 68px，占位 `width: 68px`）
   - 或：所有行使用固定宽度操作区，缺失的操作留空但宽度不变
   - 禁止：某行有"登记回款+眼睛"，另一行只有"眼睛"，导致眼睛图标左右参差
   - 检查方式：操作列内同一类型的图标（眼睛、编辑等）在所有行必须垂直对齐成一条竖线

17. **卡片组布局：按内容自适应，不强行等高**：同一行/同组排列的卡片不允许因高度不同产生大块空白，但**不允许强行拉伸卡片高度**（`flex: 1` 撑高、`space-between` 拉散内容）来制造假等高。选择以下布局策略：
   - **列内堆叠（推荐）**：把高度不同的卡片放进同一列（`stack`），每列内部自然累加高度；左右列通过内容分组平衡，矮卡片下方由同列的其他卡片填充，不单独留白。列容器设置 `align-items: start`，让两列按各自内容高度独立排列，不强制 stretch
   - **并排等高（仅当内容量相近）**：只有当同组卡片内容量确实接近时才并排，使用 Grid/Flex 的 `align-items: stretch` 让底部对齐
   - **瀑布流**：高度差异大的多个卡片可放入 CSS columns 瀑布流容器，短卡片下方自动被后续卡片填充
   - 禁止：把内容量差异大的两张卡片强行并排并用 `flex: 1` 拉伸矮卡片；禁止用 `justify-content: space-between` 把列表项拉开填满高度
   - 检查方式：卡片内部间距应自然，不出现被拉得很开的列表项；矮卡片下方不应有大块空白（通过列内堆叠或后续内容填充解决）
...
8. **沉浸动效 / Canvas**：官网、品牌、营销、作品集、创意展示页必须至少包含一个实质的动效或交互焦点（Canvas 粒子/流动渐变背景、滚动驱动视差、鼠标磁吸/3D 倾斜、入场序列、数字计数、跑马灯等），不允许整页只有静态排版。后台、工具、长阅读页不强制，只做必要状态反馈。技术目录与质量门禁见 [沉浸动效与 Canvas](motion-canvas.md)：动效必须可降级（`prefers-reduced-motion`）、性能可控（rAF、离屏停止、粒子 ≤ 150）、不遮挡内容、Canvas 不替代真实产品图片。
...
☐ 表单步骤/分组内独立卡片间距明确
☐ 汇总/确认卡未压缩在窄侧栏（含金额日期的信息完整展示）
☐ 流程成功/结果态为完整全宽面板（有容器居中内容，并隐藏无关表单/步骤）
☐ Toast/消息提示显示在顶部（非底部）
...
☐ 状态圆点为正圆且语义色对比足够
☐ 状态圆点均有背景色（无只有宽高不可见的 dot）
☐ 无引用未定义的 CSS 变量（所有 var(--xxx) 均在 :root 定义）
☐ 列表右侧列对齐（右边缘成连续竖线）
☐ 表格操作列图标垂直对齐（条件按钮用等宽占位）
☐ 卡片组按内容自适应（无强行拉伸，无大空洞）
☐ 至少一个非卡片分区方式
```
